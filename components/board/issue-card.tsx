    ...
    <ContextMenu>
      <ContextMenuTrigger asChild>
        <button
          ...
          onDragOver={
            onDropOnCard ? (event) => event.preventDefault() : undefined
          }
          onDrop={
            onDropOnCard
      ...
      <div className="mt-1 flex items-center gap-2 text-[10px] text-[var(--muted-foreground)] font-bold tabular-nums">
        {issue.commentCount ? (
          <div className="inline-flex items-center gap-1 rounded-full border border-white/10 bg-white/5 px-2 py-0.5">
      ...
      <ContextMenuContent>
        <ContextMenuItem
          onClick={() => onAddComment?.(issue._id)}
        >
          Add comment
        </ContextMenuItem>
        <ContextMenuSeparator />
